...
class FrozenCLIPEmbedder(nn.Module):
    ...
    def forward(self, text, class_index):  # [ 3 个 'A high-quality image of the automobile'], 1
        ...
        if text[0] != '':  # 'A high-quality image of the automobile'
            ...
            tmp_token = self.tokenizer([class_index], truncation=True, max_length=self.max_length, return_length=True,
            ...
            if False:
                ...
                outlier = self.outlier_embedding  # torch.Size([768])
    ...
    # [ 3 个 'A high-quality image of the shark'], 'shark', opt
    def encode(self, text, class_index, opt):
        ...
        # shark token
        tmp_token = self.tokenizer([opt.cls_1], truncation=True, max_length=self.max_length,
                                   return_length=True,
                                   return_overflowing_tokens=False, padding="max_length",
                                   return_tensors="pt")  # {'input_ids': tensor([[49406, 25258, 49407, 49407]]), 'length': tensor([3]), 'attention_mask': tensor([[1, 1, 1, 0, 0]])}
        tokens = tmp_token["input_ids"].to(self.device)  # tensor([[49406, 25258, 49407, 49407]], device='cuda:0')
        cls1_embed = deepcopy(self.transformer.text_model.embeddings.token_embedding.weight[tokens[0][1]])
        # bee token
        tmp_token = self.tokenizer([opt.cls_2], truncation=True, max_length=self.max_length,
                                   return_length=True,
                                   return_overflowing_tokens=False, padding="max_length",
                                   return_tensors="pt")  # {'input_ids': tensor([[49406, 25258, 49407, 49407]]), 'length': tensor([3]), 'attention_mask': tensor([[1, 1, 1, 0, 0]])}
        tokens = tmp_token["input_ids"].to(self.device)  # tensor([[49406, 25258, 49407, 49407]], device='cuda:0')
        cls2_embed = deepcopy(self.transformer.text_model.embeddings.token_embedding.weight[tokens[0][1]])
        a = 0.3
        self.outlier_embedding = a * cls1_embed + (1-a) * cls2_embed
...
def get_prompt(opt):
    ...
    if chozen_class[0] in ['a', 'e', 'i', 'o', 'u']:  # a
        return 'A high-quality image of the ' + opt.cls_1, opt.cls_1  # ('A high-quality image of the automobile', 'automobile')
    else:
        return 'A high-quality image of the ' + opt.cls_1, opt.cls_1
...
def main():
    ...
    model_condition = FrozenCLIPEmbedder().cuda()
                ...
                for n in trange(opt.n_iter, desc="Sampling"):  # 用于循环执行 653 次，同时显示进度条，用于跟踪任务进度。
                    for prompts in tqdm(data, desc="data"):  # ['a painting of a virus monster playing guitar', ... , 'a painting of a virus monster playing guitar']
                        ...
                        if opt.scale != 1.0:  # 7.5
                            # ['', '', ''], 0, opt
                            uc = model_condition.encode(batch_size * [""], "", opt)  # torch.Size([3, 77, 768])
                        if isinstance(prompts, tuple):
                            prompts = list(prompts)


                        # 'A high-quality image of the shark', 'shark'
                        ...
                        c = model_condition.encode(prompts, chozen_class, opt)  # torch.Size([3, 77, 768])
                        ...
                        if not opt.skip_save:  # True
                            ...
                            for x_sample in x_checked_image_torch:  # torch.Size([3, 256, 256])
                                ...
                                img.save(os.path.join('./scripts/visualize/Interpolation_Visualize/Strawberry-Fountain', '0.3_' + opt.cls_1 +'_0.7_' + opt.cls_2 + '_' + f"{base_count:05}.png"))
