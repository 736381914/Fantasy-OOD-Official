...
parser.add_argument('--ood_gene_select', type=int, default=900)
...
# 加载文本嵌入 anchor
anchor = torch.from_numpy(np.load('./token_embed_in100.npy')).cuda()  # torch.Size([100, 768])
num_classes = 100
sum_temp = 0

# 加载采样集合
data_dict = torch.from_numpy(np.load('./id_feat_in100_99epoch.npy')).cuda()  # torch.Size([100, 1000, 768])
...
kde = [KernelDensityEstimator(feat_dim=args.feat_dim, bandwidth=args.bandwidth, K=args.hamos_K, res=None) for i in range(num_classes)]
# 初始化 100 个类别 kde 中的数据
for cls_idx in range(num_classes):  # range(0, 100)
    # KernelDensityEstimator, torch.Size([1000, 768])
        ...
        if True:
            if args.shift:  # inlier
                ...
                for index1 in range(100):
                    new_dis = MultivariateNormal(torch.zeros(768).cuda(), torch.eye(768).cuda())
                    negative_samples = new_dis.rsample((1500,))
                    # torch.Size([100, 768]), torch.Size([200, 768])
                    sample_point1, boundary_point = generate_outliers(ID,
                                                                      input_index=KNN_index,
                                                                      negative_samples=negative_samples,
                                                                      ID_points_num=2,
            ...
            if index == 0:
                ood_samples = [sample_point * anchor[index].norm()]  # torch.Size([80000, 768])
            else:
                # 每个类采样了 20000 个离群值, 100 个类一共采样了 100 * 20000 = 2000000 个离群值
                ood_samples.append(sample_point * anchor[index].norm())  # [torch.Size([20000, 768]), ... , torch.Size([20000, 768])]

print("ood_samples: ", torch.stack(ood_samples).shape)  # torch.Size([100, 20000, 768]

if args.shift:
    np.save \
        ('./in100_inlier_npos_embed'+ '_noise_' + str(args.gaussian_mag_ood_gene)  + '_select_'+ str(
        args.ood_gene_select) + '_KNN_'+ str(args.K_in_knn) + '.npy', torch.stack(ood_samples).cpu().data.numpy())
else:
    np.save \
        ('./in100_outlier_npos_embed' + '_noise_' + str(args.gaussian_mag_ood_det) + '_select_' + str(
