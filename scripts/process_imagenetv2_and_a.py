...
for name in f1:
    ...
    shutil.copytree(os.path.join(
        '/home/zrf/datasets/ImageNet_V2/imagenetv2-matched-frequency-format-val', str(index)),
        os.path.join(
            '/home/zrf/datasets/ImageNet_V2/imagenetv2_processed', copy_name)
    )
    copy_name += 'a'

# for imageneta
name_list = os.listdir('/home/zrf/datasets/ImageNet_A/imagenet-a')
index = 0
for name in f1:
    # index = name_list.index(name)
    if name in name_list:
        shutil.copytree(os.path.join(
            '/home/zrf/datasets/ImageNet_A/imagenet-a', name),
            os.path.join(
                '/home/zrf/datasets/ImageNet_A/imagenet-a_processed', str(index))
        )
    index += 1
