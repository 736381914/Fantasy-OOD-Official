...
class ResNet_Model(nn.Module):
    ...
    def __init__(self, name='resnet50', num_classes=10):
        ...
        self.fc = nn.Linear(dim_in, num_classes)  # Linear(in_features=512, out_features=10, bias=True)
    ...
    def forward(self, x, vos=False):  # torch.Size([266, 3, 32, 32]), False
        if vos:
            feat = self.encoder(x)
            return feat, self.fc(feat)
        else:
            return self.fc(self.encoder(x))  # torch.Size([266, 512]) -> torch.Size([266, 10])
