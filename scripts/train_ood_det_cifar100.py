...
train_data_in = dset.CIFAR100('/home/zrf/datasets/CIFAR100', train=True, transform=train_transform)
test_data = dset.CIFAR100('/home/zrf/datasets/CIFAR100', train=False, transform=test_transform)
...
ood_data = dset.ImageFolder(root="./snapshots/txt2img-samples-cifar100-demo/" + args.my_info,
...
def train(epoch):
    ...
    for data_in, target_in in train_loader_in:
        ...
        data = torch.cat((data_in, data_ood), 0)
        target = target_in
        ...
        if args.add_class:
            ...
            loss = F.cross_entropy(x[:len(data_in)], target)
            if args.score == 'energy':

                Ec_in = torch.logsumexp(x[:len(data_in)], dim=1)
                Ec_out = torch.logsumexp(x[len(data_in):], dim=1)
                binary_labels = torch.ones(len(x)).cuda()
                binary_labels[len(data_in):] = 0
                ...
                loss += args.energy_weight * -(x[len(data_in):].mean(1) - torch.logsumexp(x[len(data_in):], dim=1)).mean()
