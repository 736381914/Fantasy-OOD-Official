...
map = ood_data.class_to_idx  # class -> idx
map1 = {}  # idx -> class
for key in list(map.keys()):
    map1[map[key]] = int(key)  # idx : class
# breakpoint()
if args.augmix:  # 0
    ood_data = dset.ImageFolder(
        root="./snapshots/txt2img-samples-in100-demo/" + args.my_info,
...
def train():
    ...
    for _, in_set in enumerate(train_loader_in):  # torch.Size([40, 3, 224, 224]), torch.Size([40])
        # print(in_set[0])
        # print(out_set[0])

        try:
            out_set = next(batch_iterator)  # torch.Size([40, 3, 224, 224]), torch.Size([40])
        except StopIteration:
            # train_loader_out.dataset.offset = np.random.randint(len(train_loader_out.dataset))
            batch_iterator = iter(train_loader_out)
            out_set = next(batch_iterator)

        data = torch.cat((in_set[0], out_set[0]), 0)  # torch.Size([80, 3, 224, 224])
        target = torch.cat((in_set[1], out_set[1]), 0)  # torch.Size([80])

        permutation_idx = torch.randperm(len(data))  # torch.Size([80]), 生成一个打乱顺序的索引序列，用于对数据进行随机重排。
        data = data[permutation_idx]  # torch.Size([80, 3, 224, 224])
        target = target[permutation_idx]  # torch.Size([80])
        data, target = data.cuda(), target.cuda()  # torch.Size([80, 3, 224, 224]), torch.Size([80])
        # breakpoint()

        # forward
        x = net(data)  # torch.Size([80, 100])

        # torch.Size([80, 100]), torch.Size([80])
        ...
        loss_avg = loss_avg * 0.8 + float(loss) * 0.2  # 0.9323701858520508
...
# Make save directory
if not os.path.exists(args.save):  # False
    os.makedirs(args.save)
if not os.path.isdir(args.save):  # False
    raise Exception('%s is not a dir' % args.save)

# 'energy_ft_sd_slope_0_weight_1_samples_inlier_all_cohesion_real_'
...
# './snapshots/energy_ft_sd/in100_wrn_s1_energy_ft_sd_slope_0_weight_1_samples_inlier_all_cohesion_real__training_results.csv'
...
# Main loop
loss_min = 100
for epoch in range(0, args.epochs):  # 0 - 99
    state['epoch'] = epoch
